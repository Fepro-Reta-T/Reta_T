...
class TorneoService:
    def __init__(self, repo: TorneoRepository):
        self.repo = repo
    ...
    async def inscribir_equipo(self, torneo_id: UUID, equipo_id: UUID) -> bool:
        return await self.repo.inscribir_equipo(torneo_id, equipo_id)

    async def retirar_equipo(self, torneo_id: UUID, equipo_id: UUID) -> bool:
        return await self.repo.retirar_equipo(torneo_id, equipo_id)

    async def listar_equipos_inscritos(self, torneo_id: UUID) -> List[Equipo]:
        return await self.repo.listar_equipos_inscritos(torneo_id)